...
#if OS2 == 2
/* Definition of FINDFILEBUF has changed with OS/2 2.0 */
FILEFINDBUF3 pBuf;
#else
...
ULONG num_found;		/* number of directory entries found/to find */
/* APIRET rc; */
...

{
	...
	while (index >= 0 && (path[index] != '/' &&
		...
		--index;
	path[index+1] = 0;
#if OS2 != 2 /* You don't need the extension .* under OS/2 2.x */
...
#else
	if ( DosFindFirst(fname, &hDir, FILE_NORMAL|FILE_DIRECTORY,
			  &pBuf, sizeof(pBuf), &num_found, FIL_STANDARD ) != 0)
		return(NULL);
#endif		
...

{
   ...
   APIRET rc;
	...
	if ( (rc = DosFindNext(hDir, (void *)&pBuf, sizeof(pBuf), &num_found)) != 0)
		return(NULL);
